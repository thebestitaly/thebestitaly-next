...
// API Route per destinazioni: src/app/api/directus/destinations/route.ts
import { NextResponse } from 'next/server';

export async function GET() {
  try {
    const directusUrl = process.env.DIRECTUS_URL;
    const response = await fetch(`${directusUrl}/items/destinations?fields=id,slug,name&limit=-1`);
    const data = await response.json();
